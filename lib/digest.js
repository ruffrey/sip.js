...
// TODO: FIXME: will not scale pass a single process!
...
/**
 * Constructs a nonce and returns the expected timestamp if successful; otherwise
 * returns undefined.
 * @returns {Date} ts - the timestamp from a nonce, if reconstructed
 */
function extractNonceTimestamp(nonce, tag) {
  ...
  const reconstructedNonce = generateNonce(tag, ts);
  const nonceIsExpected = reconstructedNonce === nonce;
...
exports.authenticateRequest = function authenticateRequest(ctx, rq, creds) {
  if (!rq.headers) {
    // console.log('missing headers on rq', { ctx, rq, creds });
    return false;
  }
